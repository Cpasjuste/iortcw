...
Q_EXPORT void cg_dllEntry( intptr_t (QDECL *syscallptr)( intptr_t arg,... ) ) {
	syscall = syscallptr;
}
...
//----(SA)	added
void    cg_trap_S_StartSoundEx( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int flags ) {
...
void    cg_trap_S_AddLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, int range, sfxHandle_t sfx, int volume ) {
...
// not in use
//void    cg_trap_S_AddRealLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, int range, sfxHandle_t sfx, int volume ) {
...
//----(SA)	added
void    cg_trap_S_StopStreamingSound( int entityNum ) {
...
int     cg_trap_S_GetVoiceAmplitude( int entityNum ) {
...
sfxHandle_t cg_trap_S_RegisterSound( const char *sample ) {
	CG_DrawInformation();
	return syscall( CG_S_REGISTERSOUND, sample );
}

void    cg_trap_S_StartBackgroundTrack( const char *intro, const char *loop, int fadeupTime ) {
	syscall( CG_S_STARTBACKGROUNDTRACK, intro, loop, fadeupTime );
}

//----(SA)	added
void    cg_trap_S_FadeBackgroundTrack( float targetvol, int time, int num ) {   // yes, i know.  fadebackground coming in, fadestreaming going out.  will have to see where functionality leads...
	syscall( CG_S_FADESTREAMINGSOUND, PASSFLOAT( targetvol ), time, num ); // 'num' is '0' if it's music, '1' if it's "all streaming sounds"
}

void    cg_trap_S_FadeAllSound( float targetvol, int time ) {
	syscall( CG_S_FADEALLSOUNDS, PASSFLOAT( targetvol ), time );
}
//----(SA)	end

void    cg_trap_S_StartStreamingSound( const char *intro, const char *loop, int entnum, int channel, int attenuation ) {
...
//----(SA)	added
qboolean cg_trap_R_GetSkinModel( qhandle_t skinid, const char *type, char *name ) {
	return syscall( CG_R_GETSKINMODEL, skinid, type, name );
}

qhandle_t cg_trap_R_GetShaderFromModel( qhandle_t modelid, int surfnum, int withlightmap ) {
...
void    cg_trap_RB_ZombieFXAddNewHit( int entityNum, const vec3_t hitPos, const vec3_t hitDir ) {
	syscall( CG_RB_ZOMBIEFXADDNEWHIT, entityNum, hitPos, hitDir );
}
// done.

void    cg_trap_R_AddLightToScene( const vec3_t org, float intensity, float r, float g, float b, int overdraw ) {
	syscall( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT( intensity ), PASSFLOAT( r ), PASSFLOAT( g ), PASSFLOAT( b ), overdraw );
}

//----(SA)
void    cg_trap_R_AddCoronaToScene( const vec3_t org, float r, float g, float b, float scale, int id, int flags ) {
	syscall( CG_R_ADDCORONATOSCENE, org, PASSFLOAT( r ), PASSFLOAT( g ), PASSFLOAT( b ), PASSFLOAT( scale ), id, flags );
}
//----(SA)

//----(SA)
void    cg_trap_R_SetFog( int fogvar, int var1, int var2, float r, float g, float b, float density ) {
...
void    cg_trap_R_DrawStretchPicGradient(  float x, float y, float w, float h,
										float s1, float t1, float s2, float t2, qhandle_t hShader,
...
int     cg_trap_R_LerpTag( orientation_t *tag, const refEntity_t *refent, const char *tagName, int startIndex ) {
...
void        cg_trap_SetUserCmdValue( int stateValue, int holdableValue, float sensitivityScale, int cld ) {    //----(SA)	// NERVE - SMF - added cld
...
qboolean cg_trap_loadCamera( int camNum, const char *name ) {
	return syscall( CG_LOADCAMERA, camNum, name );
}

void cg_trap_startCamera( int camNum, int time ) {
	syscall( CG_STARTCAMERA, camNum, time );
}

//----(SA)	added
void cg_trap_stopCamera( int camNum ) {
	syscall( CG_STOPCAMERA, camNum );
}
//----(SA)	end

qboolean cg_trap_getCameraInfo( int camNum, int time, vec3_t *origin, vec3_t *angles, float *fov ) {
...
void cg_trap_SendMoveSpeedsToGame( int entnum, char *movespeeds ) {
...
void cg_trap_UI_Popup( const char *arg0 ) {
	syscall( CG_INGAME_POPUP, arg0 );
}

// NERVE - SMF
void cg_trap_UI_LimboChat( const char *arg0 ) {
	syscall( CG_LIMBOCHAT, arg0 );
}

void cg_trap_UI_ClosePopup( const char *arg0 ) {
	syscall( CG_INGAME_CLOSEPOPUP, arg0 );
}
// -NERVE - SMF

qboolean cg_trap_GetModelInfo( int clientNum, char *modelName, animModelInfo_t **modelInfo ) {
	return syscall( CG_GETMODELINFO, clientNum, modelName, modelInfo );
}

// New in IORTCW
void *cg_trap_Alloc( int size ) {
